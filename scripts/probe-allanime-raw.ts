...
async function rawEpisode(showId: string, ep: string) {
  ...
  const getRes = await fetch(
    `${ALLANIME_API}?variables=${encodeURIComponent(JSON.stringify(variables))}&extensions=${encodeURIComponent(JSON.stringify(extensions))}`,
    {
  ...
  const decryptedGet = JSON.parse(decryptAllAnimePayload(getJson));
  ...
  const decryptedPost = JSON.parse(decryptAllAnimePayload(postJson));
...
async function main() {
  ...
  if (show) await rawEpisode(show.id, "1");
}
