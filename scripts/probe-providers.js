...
async function main() {
    console.log("=== HiAnime ===");
    try {
        const h = new extensions_1.ANIME.Hianime();
    ...
    try {
        ...
        console.log("allanime search", search.results.slice(0, 5).map((item) => item.title));
        const show = search.results.find((item) => item.title.toLowerCase() === "naruto") ?? search.results[0];
        console.log("show", show?.title, show?.id);
        if (!show)
            return;
        const episodes = await allanime_provider_1.allanimeProvider.episodes(show.id);
        const episode = episodes.episodes[0];
        console.log("episode", episode?.id);
        const servers = await allanime_provider_1.allanimeProvider.servers(episode.id);
        console.log("servers", servers.servers);
        const sources = await allanime_provider_1.allanimeProvider.sources(episode.id, servers.servers[0]?.id ?? "default", "sub");
