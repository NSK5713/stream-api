...
async function main() {
  ...
  for (const [name, id] of ids) {
    const eps = await allanimeProvider.episodes(id);
    console.log(
      name,
      "count",
      eps.episodes.length,
      "max",
      eps.episodes.at(-1)?.number,
      "has12",
      eps.episodes.some((e) => e.number === 12),
    );
  }

  const ep12Id = "XjZScvcpvP9hizG8v@12";
  const sources = await allanimeProvider.sources(ep12Id, "default", "sub");
  console.log("\nEp12 sources:");
