...
async function main() {
  ...
  const chainSearch = await consumetMultiProvider.search("No Game, No Life");
  for (const item of chainSearch.results.slice(0, 10)) {
    console.log(`  ${item.id} — ${item.title}`);
  }

  const best =
    chainSearch.results.find((item) => item.id.startsWith("allanime:")) ??
  ...
  const episodeList = await consumetMultiProvider.episodes(best.id, {
  ...
  if (!ep12) {
    ...
    for (const item of chainSearch.results.filter((r) => r.id.startsWith("allanime:")).slice(0, 5)) {
      const eps = await consumetMultiProvider.episodes(item.id);
  ...
  console.log("\n=== Sources ep 12 sub ===");
  try {
    const sources = await consumetMultiProvider.sources(ep12.id, "default", "sub");
