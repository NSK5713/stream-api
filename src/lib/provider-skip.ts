...
/**
 * Safely parses skip range with full TS protection
 */
...
/**
 * Reference duration helper
 */
export function providerSkipReferenceDuration(
...
/**
 * Converts raw payload → typed skip object
 */
...
function parseHianimeEpisodeRef(episodeId: string): {
  watchUrl: string;
  episodeNumericId: string;
} | null {
...
/**
 * Extract server id from HiAnime HTML
 */
function retrieveHianimeServerId(
  ...
  const subOrDub =
    category === "raw" ? "raw" : category === "dub" ? "dub" : "sub";
  ...
  const pickFromBlock = (useRaw: boolean): string | null => {
    ...
    $(
      `.ps_-block.ps_-block-sub.servers-${blockSuffix} > .ps__-list .server-item`
    ).each((_, el) => {
      const item = $(el);
...
export function mapStreamServerToHianimeIndex(server: string): number {
  return HIANIME_SERVER_INDEX[server.toLowerCase()] ?? 1;
}

/**
 * Fetch skip times from HiAnime
 */
export async function fetchHianimeProviderSkip(
  ...
  try {
    ...
    // 2. Get sources (skip data lives here)
    ...
    console.error("Skip fetch failed:", err);
    return null;
  }
}